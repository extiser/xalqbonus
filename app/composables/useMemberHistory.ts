...
/**
 * История операций участника: первая страница, тихая перезагрузка и догрузка прокруткой.
 *
...
export const useMemberHistory = (readInitData: () => string) => {
  ...
  /** Метка следующей страницы. `null` — показано всё, список кончается. */
  const nextCursor = ref<string | null>(null);

  /** Догрузка в пути: второй сигнал с конца списка не приведёт ту же страницу дважды. */
   ...
   *
   * Для перечитывания главной — при возврате на неё и из фона: данные обычно те же самые,
   * а список на время запроса собирался бы заново и моргал — перерисовка прочитанного
   * (issue #107).
   *
   * Отказ не перехватывается и не гасит список: показанные строки верны, увести их
   * в ошибку в ответ на перечитывание — то же самое, что стереть баланс. Что делать
   * с отказом — решает вызывающий.
   */
