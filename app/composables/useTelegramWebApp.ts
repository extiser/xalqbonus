...
export type TelegramWebApp = {
  ...
  ready: () => void;
  expand: () => void;
  /** Закрывает окно приложения — сброс сессии из профиля (issue #216). */
  close: () => void;
  /**
   * Вопрос Telegram при закрытии приложения жестом или свайпом — своим текстом клиента. Включён,
   * пока в каталоге лежит корзина: она не запоминается (issue #234). Доступны с Bot API 6.2 —
   * у клиентов постарше полей нет, поэтому необязательные.
   */
...
/**
 * Стирает копию подписанной строки — сброс сессии из профиля (issue #216). Следующее открытие
 * приложения начнётся со строки, которую выдаст Telegram, а не с этой.
 */
export const forgetInitData = (): void => {
  try {
    sessionStore()?.removeItem(INIT_DATA_KEY);
  } catch {
    // Запрещённое хранилище: копии в нём и не было.
  }
};
