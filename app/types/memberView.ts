...
export interface MemberProductView {
  ...
  available?: number;
  /**
   * Товара нет в офисе витрины: «Нет в Кадышева» на пунктирной плашке вместо счётчика.
   * Есть — плитка приглушённая и не нажимается (issue #234).
   */
  missing?: string;
}
...
export interface MemberCatalogOfficeView {
  ...
  name: string;
  address: string;
  /** Остаток нажатого товара в этом офисе, «5 шт», — справа от адреса. Нет — строки без остатка. */
  stock?: string;
}
