...
/**
 * Обложка подарка в форме «Вручить» (issue #219): выбор файла и превью в рамке 16:9 с обрезкой
 * по краям — так, как её увидит водитель в шторке подарка
 ...
 * с обложкой, и до «Вручить» файл живёт только в форме. Ограничения — из `shared/photo.ts`,
...
<template>
  <div class="flex flex-wrap items-start gap-6">
    <div class="w-64 space-y-2">
      <div class="aspect-video w-full overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
        <img v-if="previewUrl" :src="previewUrl" alt="Обложка подарка" class="h-full w-full object-cover" />
    ...
    <div class="min-w-64 flex-1 space-y-2">
      <label class="block">
        <span class="mb-1 block text-sm font-medium text-slate-700">Обложка</span>
        ...
        <span class="mt-1 block text-sm text-slate-500">
          Необязательно. JPEG, PNG или WebP, до {{ MAX_PHOTO_MB }} МБ. С обложкой сообщение водителю
          уходит фото с подписью; в приложении обложка видна в рамке 16:9, края обрезаются.
        </span>
      </label>
