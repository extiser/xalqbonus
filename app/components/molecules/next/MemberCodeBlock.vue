 ...
 * а сотрудник читает его с чужого телефона, часто на солнце. Подпись — 13/500 зелёным,
 * одна у заказа и награды (шкала шрифтов, «Подпись над кодом выдачи»).
 *
...
<template>
  <div class="flex flex-col" :class="WRAP_CLASSES[size]">
    <span class="text-[13px] font-medium text-xb-green">{{ title }}</span>
    <span class="font-unbounded font-bold tabular-nums text-xb-green" :class="CODE_CLASSES[size]">{{ code }}</span>
  </div>
</template>
