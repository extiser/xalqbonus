 ...
 * попадает примерно в одиннадцать прогонов подряд. Суммы отличаются от чисел на порядок.
 */
defineProps<{
  state: LoadState;
...
<template>
  <MoleculesSectionPanel
    ...
    note="Различные заказы, начисления и пропущенное — по таблицам данных, по времени события. Счётчики прогонов здесь не складываются: один заказ попадает в десяток прогонов подряд."
  >
