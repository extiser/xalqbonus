...
import type { TripsNote } from '#shared/types/miniapp';
 ...
 *
 * Строка «Поездки учтены до…» стоит сразу под ним намеренно: поездки приезжают прогоном,
...
defineProps<{
  ...
  name: string;
  /**
   * Есть всегда: без успешных прогонов в ней «данные ещё не поступали». Без строки кнопка
   * обновления стояла бы одна, без единого слова.
   */
  tripsNote: TripsNote;
  ...
  <section class="flex flex-col gap-4">
    <div class="flex flex-col gap-1">
      ...
      <div class="mt-1 flex items-center gap-2">
        <!--
          Отставание больше суток — предупреждением, а не серой подписью: в этом состоянии
          водитель прав, что поездки не приходят, и экран не должен делать вид, что всё в порядке.
        -->
        <p
          :class="
            tripsNote.stale
              ? 'rounded-md bg-amber-50 px-2 py-1 text-sm font-medium text-amber-800'
