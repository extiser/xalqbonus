...
/**
 * Рассылки — не для менеджера.
 *
 * Проверка здесь дублирует серверную (`server/api/mailings/*`) и ничего не решает: набранный
...
export default defineNuxtRouteMiddleware(() => {
  ...
  if (employee.value !== null && !MAILING_ROLES.includes(employee.value.role)) {
