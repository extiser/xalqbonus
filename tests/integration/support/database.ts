...
/**
 * Отдаёт уборке человека, заведённого не фикстурой, а самим кодом под тестом, — демо-водителя
 * (issue #205). Строки, которые ссылаются на него сверх обычного набора, — привязку,
 * удостоверение, запись демо-зрителя, — убирает `support/demo.ts`, и раньше этой уборки.
 */
export const trackTestPerson = (personId: string): void => {
