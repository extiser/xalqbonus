...
не оформлена и скриптом не применяется. Ниже — **образец** `server`-блока как справка
тому, кто правит машину руками. Блок `xalq.bonusbot.uz` на машине уже есть — вместе
с редиректом с 80 порта и выпущенным сертификатом; добавляется в него один `location /`.
...
server {
    ...
    # Webhook старого бота — уже существующий блок. Он не правится и не переставляется:
    # префикс совпадает точнее, чем `/`, и nginx выберет его независимо от порядка,
    # но трогать живой webhook ради косметики незачем.
    location /xalqbonusbot/ {
        proxy_pass http://127.0.0.1:4000/;
    }

    # Приложение. 3003 — порт, опубликованный стеком на петлевом адресе.
    location / {
...
и на машине может уже существовать. Второй раз её объявлять нельзя, nginx откажется
стартовать.
