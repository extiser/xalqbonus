...
и редиректом с 80 порта, — и в нём уже стоит заглушка:
...
Это она и отдаёт 404, а не отсутствие правила. Её нужно **заменить**, а не дописать
рядом: двух `location /` в одном `server`-блоке nginx не принимает — `nginx -t` упадёт
...
и на машине этой карты **нет**. Webhook старого бота обходится без неё — у него в заголовке
