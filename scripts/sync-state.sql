...
SELECT run."kind",
       ...
       run."started_at",
       detail."pages"                   AS "страниц",
       detail."profiles_seen"           AS "профилей",
       detail."profiles_inserted"       AS "новых",
       detail."profiles_updated"        AS "подтверждено",
       -- Строк ответа больше числа профилей у полного обхода: половины куска, взятого
       -- с двух концов, перекрываются намеренно. Это цена нарезки, а не размер парка.
       detail."response_rows"           AS "строк ответа",
...
-- в раз и не может записать. Ссылка — идентификатор заказа либо `словарь=значение`,
