...
switch (result.outcome) {
  ...
  // Строка своя, а не `invite_driver_link_exists` из словаря бота: тот написан приглашённому
  // и советует сообщить выписавшему ссылку, а здесь читает тот, кто выкатывает.
  case 'driver_link_exists':
    log.error(
      'телефон за активной водительской привязкой: водителем и сотрудником одновременно быть ' +
        'нельзя — закройте привязку с причиной operator или заведите владельца на другой номер',
    );
    break;
}
