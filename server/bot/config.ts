...
/**
 * Адрес Mini App — тот, что открывает кнопка под приветствием `/start`.
 *
 * Пустая строка — рабочее состояние, а не ошибка: Telegram открывает Mini App только
 * по `https`, а локальный стенд живёт на `http://localhost` и адреса снаружи не имеет
 * вовсе. На такой машине `/start` отвечает одним приветствием, без кнопки, а приложение
 * открывается кнопкой меню, заведённой в BotFather.
 *
 * Отдельной переменной, а не вычислением из `TG_WEBHOOK_URL`: домен у них сегодня общий
 * (docs/decisions.md → «Mini App живёт на том же домене, что приложение»), но выводить
 * один адрес из другого значит завести связь, о которой узнают в тот день, когда она
