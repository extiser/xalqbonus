...
// Итог окна и вскрытие сундуков акций — четвёртая очередь, своя по той же причине: волна в тысячи участников
...
campaignsWorker.on('failed', (job, error) => {
  // Исходы пишутся только туда, где их нет, а вскрывается только неоткрытое, поэтому упавший
  // прогон ничего не портит: следующий через пятнадцать минут возьмёт тех же участников.
  log.error('прогон очереди акций упал', { kind: job?.data.kind, error: error.message });
});
...
const shutdown = async (signal: string): Promise<void> => {
  ...
  // Итог окна и вскрытие — по той же причине: оборванный прогон оставит часть участников
