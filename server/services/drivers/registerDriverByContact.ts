...
type AttemptPhone = { raw: string | null; e164: string | null };
 ...
 *
 * Обе колонки — `null`, а не пустая строка: пустая означала бы «номер был, и он пустой»,
 * и запрос «попытки без номера» пришлось бы писать с оглядкой на два значения сразу.
 */
const NO_PHONE: AttemptPhone = { raw: null, e164: null };
...
export const registerDriverByContact = async (
  ...
  return {
    ...
    driver: {
      ...
      // Регистрирующийся не демо: у демо-водителя телефона нет, и по контакту его не найти.
      isDemo: false,
    },
  };
};
