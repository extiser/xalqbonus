...
export const sendNotification = async (
  ...
  //
  // На боевой машине бот есть всегда, и пустой токен означает там не режим, а аварию:
  // сломанное окружение, из-за которого молча пропадают все уведомления подряд. Поэтому
  // строка идёт ошибкой, а не предупреждением, — `warn` в логе воркера тонет среди штатных
  // строк, — и говорит прямо, что сообщение потеряно: задание завершается успешно,
  // повторов не получает и следа после себя не оставляет.
  if (token === '') {
    log.error('уведомление потеряно безвозвратно: TG_BOT_TOKEN пуст, отправлять нечем', {
      template,
      personId,
    });
