...
export default defineEventHandler(async (event): Promise<EmployeePasswordResponse> => {
  ...
  // Отказ доменного правила, а не двери: демо-зритель в роли менеджера доступ имеет, а пароля
  // у демо-учётки не бывает (issue #205). Пункта пароля у него на экране нет, и досюда такой
  // запрос доходит только в обход экрана.
  if (outcome === 'demo_account') {
    throw createError({
      statusCode: 403,
