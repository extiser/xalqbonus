 ...
 * перевода, и строки счёта берутся по индексу `(account_id, id)`.
 *
 * Порядок — по времени операции, а не по порядку внесения в журнал. Это не одно и то же:
 * `occurred_at` задаётся событием — временем завершения поездки, датой раздачи по кампании,
 * периодом перепроверки, — а `created_at` временем вставки, и запись задним числом
 * разводит их на сутки и месяцы. На экране показано время операции, и список,
 * отсортированный не по тому, что в нём написано, читается как ошибка начисления.
 *
 * `entry."id"` остаётся вторым ключом: он добивает совпавшие моменты, без него две
 * страницы одного счёта могут показать одну и ту же строку дважды.
 *
 * Индекса под этот порядок нет и не нужно: сортировка идёт по колонке присоединённой
 * таблицы, индексом её не закрыть, а записей на одном счёте сотни — цена сортировки
 * в памяти на таком объёме нулевая.
 *
