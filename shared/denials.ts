...
export type ServerDenialCode =
  ...
  | 'role_not_allowed'
  /**
   * Демо-сотрудник на ручке, не открытой демо (issue #205): под демо-учёткой входит чужой
   * человек, которому показывают программу, и до живых водителей ему не дотянуться ничем.
   */
  | 'demo_not_allowed'
  /**
   * Демо-сущность меняет не владелец (issue #212): демо — витрина программы, которую показывает
   * владелец, и правка её чужими руками ломает показ. Остальные роли видят демо с пометкой.
   */
  | 'demo_owner_only'
...
const SERVER_DENIAL_TEXTS: DenialTexts<ServerDenialCode> = {
  ...
  demo_not_allowed: {
    ru: 'В демо-аккаунте это действие недоступно.',
  },
  demo_owner_only: {
    ru: 'Демо может менять только владелец',
  },
  invalid_credentials: {
...
type DenialValueNames = {
  ...
  demo_not_allowed: never;
  demo_owner_only: never;
