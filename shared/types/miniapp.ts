...
export type MiniAppEmployeeScreen = {
  ...
  fullName: string;
  role: EmployeeRole;
  /** Телефон учётки, `employees.phone_e164` — в профиле и в пояснении формы пароля. */
  phone: FormattedPhone;
  /**
   * `user.id` из проверенной `initData`, строкой. У демо-менеджера — Telegram зрителя:
   * своего у демо-учётки нет.
   */
  telegramId: string;
  /**
   * Пароль для веба задан. Нет — в профиле «Пароль для входа с компьютера → Задать»; задан —
   * «Задан» без перехода: смена пароля живёт в вебе, на `/password` (issue #130, #250).
   */
  passwordSet: boolean;
  /**
   * Работающие офисы сотрудника, архивных нет (issue #250). Пусто — менеджер ни к одному офису
   * не привязан, и экран говорит об этом словами.
   */
