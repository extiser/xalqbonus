...
export type TripsNote = {
  /** Готовая строка: день подписан на сервере — «вчера» или датой, когда прогон не сегодняшний. */
  text: string;
  /**
...
export type MiniAppMemberScreen = {
  ...
  balance: string;
  /**
   * «Поездки учтены до 14:32» — время последнего **успешного** прогона заказов.
   *
   * Пусто, когда успешных прогонов не было ни одного: строки нет вовсе. Время
   * неудачной попытки в роли отметки — это ровно то враньё, от которого
   * экран и заводится.
   */
