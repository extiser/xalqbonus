 ...
 *
 * Счётчиков, которые различным подсчётом не берутся, здесь нет намеренно: «вне программы»
 * живёт только в разборе отдельного прогона, и сумма его по периоду отличается от числа
 * заказов на порядок — окно живой синхронизации приносит один заказ примерно в одиннадцать
 * прогонов подряд.
 */
